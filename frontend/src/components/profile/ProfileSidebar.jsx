...
const ProfileSidebar = ({active, setActive}) => {
  ...
  return (
    <div className="w-full bg-white shadow-sm rounded-[10px] pt-8 p-4">
      <div className="flex items-center cursor-pointer w-full mb-5" onClick={() => setActive(1)}>
        <PiUserThin size={20} color={active === 1 ? ('red') : ''}/>
        <span className={`${active === 1 ? 'text-[red]' : ''} pl-3 font-thin hidden md:block`}>
            Profile
        </span>
      </div>
      <div className="flex items-center cursor-pointer w-full mb-5" onClick={() => setActive(2) || navigate('/inbox')}>
        <GoInbox size={20} color={active === 2 ? ('red') : ''}/>
        <span className={`${active === 2 ? 'text-[red]' : ''} pl-3 font-thin hidden md:block`}>
            Inbox
        </span>
      </div>
      <div className="flex items-center cursor-pointer w-full mb-5" onClick={() => setActive(3)}>
        <PiAddressBookThin size={20} color={active === 3 ? ('red') : ''}/>
        <span className={`${active === 3 ? 'text-[red]' : ''} pl-3 font-thin hidden md:block`}>
            Address
        </span>
      </div>
